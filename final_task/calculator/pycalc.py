...
def replace_const(st,const_dict):
    p = re.compile("[a-z]+")
    edges=[(m.start(),m.end(),m.group()) for m in p.finditer(st) if m.group() in const_dict]
    edges=sorted(edges)
...
def split_by_quotes(st, basic_operators, compound_operators,is_func=False):
    ...
    split_arr=[]
    if st[0:edges[0]]!="":
        split_arr=split_arr+split_simple_exp(st[0:edges[0]],basic_operators, compound_operators)
    ...
    for ind,i in enumerate(edges[:-1]):
        ...
        if st[i]=="(":
            if (st[i-1].isalpha()) or (st[i-1].isdigit()):
                ...
                tmp_st=split_by_quotes(tmp_st,basic_operators, compound_operators)
            split_arr.append(tmp_st)
        else:
            tmp_st=split_simple_exp(tmp_st,basic_operators, compound_operators)
            split_arr=split_arr+tmp_st

    if st[edges[-1]+1:len(st)]!="":
        split_arr=split_arr+split_simple_exp(st[edges[-1]+1:len(st)],basic_operators, compound_operators)
...
def calc_rec(exp_arr,func_dict,priority_dict,operators):
    ...
    if type (exp_arr)==list:
        ...
        #numbers to float
        exp_arr=[float(i) if((type(i)==str) and i.replace('.','',1).isdigit()) else i for i in exp_arr ]
        
        #calculate brackets
        exp_arr=[calc_rec(i,func_dict,priority_dict,operators) if type(i)==list else i for i in exp_arr ]
        
        #exit recursion with error
        if "error" in exp_arr:
            return "error"

        func_ind=[ind for ind,i in enumerate(exp_arr) if (type(i)==str) and (i in func_dict.keys())]
        ...
        op_ind=[(priority_dict[i],ind) for ind,i in enumerate(exp_arr) if (type(i)==str) and i in operators.keys()]
        drop_arr=[]
        for i in op_ind:
            ind=i[1]
        ...
        #calculate operators
        op_ind=[(priority_dict[i],ind) for ind,i in enumerate(exp_arr) if (type(i)==str) and i in operators.keys()]
        op_ind=sorted(op_ind, key= lambda x: key_by_pripority(x,priority_dict), reverse=True)
        while (len(op_ind)>0):
            i=op_ind[0][1]
            arg_1=exp_arr[i-1]
            arg_2=exp_arr[i+1]
